...
export const Favourite: FC<{ slug:string, event:Event, size?:string }> = ({ slug, event, size }) => {
  ...
  useEffect(() => {
    const removeEvent = translation?.removeFromFavourites?.replace('{{EVENT}}', event.title) ?? '';
    const addEvent = translation?.addToFavourites?.replace('{{EVENT}}', event.title) ?? '';
    setAriaTranslation(isFavourite ? removeEvent : addEvent);
  }, [isFavourite, event, translation])
  ...
  return (
    <div className="relative h-3.5 z-50 pt-1 pl-2">
      <button onClick={addToFavourites} className='p-10 absolute right-0 bottom-0' aria-label={ariaTranslation}>
